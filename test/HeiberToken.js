...
contract('HeiberToken', function(accounts) {
    var tokenInstance;
    ...
    it('initialises the contract with the correct values', function() {
        return HeiberToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Heiber Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'HBR', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Heiber Token v1.0', 'has the correct standard')
        });
    });
